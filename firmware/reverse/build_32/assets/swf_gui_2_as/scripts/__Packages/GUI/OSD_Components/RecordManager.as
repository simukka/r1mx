class GUI.OSD_Components.RecordManager
{
         ...
         case "IDLE":
            if(!this.__isBusy)
            {
               ...
               if(this.__gpdb.paramGetBoolean("GUI.RECORD.PRERECORD.ENABLED") && this.__gpdb.paramGetBoolean("GUI.RECORD.PRERECORD.REQUESTED"))
               {
                  ...
                  var _loc4_ = this.__gpdb.paramGetNumber("GUI.RECORD.PRERECORD.DURATION");
   ...
   function SetSystemRecordParams(name, value)
   {
      ...
      {
         var _loc8_ = this.GuiRecordMode();
         var _loc2_ = undefined;
         var _loc7_ = this.__gpdb.paramGetNumber("PROJECT.MODE_MATRIX.FRAME_RATE");
         var _loc5_ = undefined;
         var _loc3_ = undefined;
   ...
   function ConfigureSysRampParams()
   {
      if(this.__gpdb.paramGetBoolean("VIDEO.RECORD.VARISPEED.ENABLED"))
      {
         ...
         {
            var _loc2_ = this.__gpdb.paramGetNumber("VIDEO.RECORD.FRAME_RATE.REQUESTED");
            var _loc3_ = this.__gpdb.paramGetNumber("GUI.RECORD.VARISPEED.RAMP.END_FRAME_RATE");
   ...
   function SelectivelyEnableMagnification(enable)
   {
      if(enable)
      {
         var _loc4_ = this.__gpdb.paramGetBoolean("VIDEO.RECORD.REQUESTED");
         var _loc3_ = this.__gpdb.paramGetBoolean("VIDEO.PLAYBACK.REQUESTED");
         if(_loc3_)
         {
            enable = false;
         }
