class GUI.OSD_Components.ProfileMgr
{
   ...
   function ExportCurrentProfile(type)
   {
      ...
      if(_loc2_ != "")
      {
         var _loc3_ = this.NewProfileName(type);
   ...
   function NewProfileName(type)
   {
      ...
      if(_loc11_ != undefined && _loc11_ != "")
      {
         ...
         {
            var _loc4_ = _loc10_[_loc5_];
            var _loc9_ = _loc4_.lastIndexOf(",") + 1;
            var _loc3_ = _loc4_.slice(_loc9_,_loc4_.length);
            if(0 == _loc3_.lastIndexOf("PROFILE_"))
            {
               var _loc7_ = _loc3_.slice(8,_loc3_.length);
               var _loc6_ = Number(_loc7_);
               if(_loc6_ > _loc8_)
               {
                  _loc8_ = _loc6_;
               }
            ...
            if(!this.__mustReboot)
            {
               switch(value)
               {
                  case "IDLE":
                     break;
                  case "RESTORE_OK":
                     var _loc6_ = this.__gpdb.paramGet("SYSTEM.PROFILE.RESTORE.TYPE");
                     if(this.clReq)
                     {
