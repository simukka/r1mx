class GUI.OSD_Components.Widget_Focus
{
   ...
   function SetFocus(distance)
   {
      ...
      else
      {
         if(this.unit == "METRIC")
         {
            var _loc8_ = distance / 100;
            _loc3_ = _global.ShowDecimalPlaces(_loc8_,2,true);
            if(_loc3_.indexOf(".") > -1)
            {
               var _loc7_ = _loc3_.split(".",2);
               _loc3_ = _loc7_[0] + "," + _loc7_[1];
            }
            this.__meter._visible = true;
            this.__DISTANCE.text = _loc3_;
            var _loc10_ = this.__DISTANCE.getTextFormat();
            var _loc11_ = _loc10_.getTextExtent(this.__DISTANCE.text).width;
            this.__meter._x = this.__DISTANCE._x + _loc11_ + 3;
         }
         else
         {
            var _loc5_ = distance % 12;
            var _loc4_ = (distance - _loc5_) / 12;
