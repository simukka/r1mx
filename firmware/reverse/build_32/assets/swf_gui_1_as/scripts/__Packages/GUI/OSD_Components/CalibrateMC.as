class GUI.OSD_Components.CalibrateMC extends GUI.OSD_Components.FullScreenMC
{
   ...
   function HandleUserCalRequest()
   {
      ...
      {
         var _loc6_ = "NONE" == this.__gpdb.paramGet("MEDIA.DIGMAG.ACTIVEDRIVE");
         if(_loc6_)
         {
         ...
         {
            var _loc3_ = Number(this.__gpdb.paramGet("MEDIA.DIGMAG.CAPACITY.TOTAL_MB"));
            var _loc4_ = Number(this.__gpdb.paramGet("MEDIA.DIGMAG.CAPACITY.REMAINING_MB"));
            var _loc5_ = Math.floor(100 * _loc4_ / _loc3_);
            if(_loc5_ < 10)
            {
   ...
   function Update(name, value)
   {
      switch(name)
      {
         case "CALIBRATE.CAPTURE_CURRENTFRAME":
            ...
            {
               var _loc5_ = this.__gpdb.paramGetNumber("CALIBRATE.CAPTURE_FRAMECOUNT");
               var _loc6_ = _loc5_ - Number(value);
