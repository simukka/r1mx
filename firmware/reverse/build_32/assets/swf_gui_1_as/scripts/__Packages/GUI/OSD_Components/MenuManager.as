class GUI.OSD_Components.MenuManager
{
   ...
   function HandleShutter(param, value)
   {
      ...
      else if(value == "DEGREES")
      {
         ...
         var _loc5_ = this.__gpdb.paramGetNumber("GUI.RECORD.SHUTTER_SPEED");
         var _loc4_ = Math.round(Number(this.__gpdb.paramGet("PROJECT.MODE_MATRIX.FRAME_RATE")));
         if(_loc5_ < _loc4_)
         {
   ...
   function PushMenuPanel(name)
   {
      ...
      if(_loc3_ != null)
      {
         ...
         {
            var _loc6_ = _loc3_.IsEmptyGizmoGrid();
            if(!_loc6_)
            {
               if(_loc5_ != null)
               {
