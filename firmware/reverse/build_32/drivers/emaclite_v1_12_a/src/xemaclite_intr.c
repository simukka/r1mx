/* $Id: xemaclite_intr.c,v 1.1 2007/11/30 11:43:03 svemula Exp $ */
/******************************************************************************
*
...
int XEmacLite_EnableInterrupts(XEmacLite * InstancePtr)
{
	...
	XEmacLite_mWriteReg(BaseAddress,
				XEL_TSR_OFFSET + XEL_BUFFER_OFFSET, Register);
	...
	Register |= XEL_RSR_RECV_IE_MASK;
	XEmacLite_mWriteReg(BaseAddress,
				XEL_RSR_OFFSET + XEL_BUFFER_OFFSET, Register);
...
void XEmacLite_InterruptHandler(void *InstancePtr)
{
	...
	if ((XEmacLite_mIsTxDone(BaseAddress) == TRUE) &&
		 ...
		 */
		Register = XEmacLite_mGetTxActive(BaseAddress);
		Register &= ~XEL_TSR_XMIT_ACTIVE_MASK;
		XEmacLite_mSetTxActive(BaseAddress, Register);
	}
	if ((XEmacLite_mIsTxDone(BaseAddress + XEL_BUFFER_OFFSET) == TRUE) &&
		((XEmacLite_mGetTxActive(BaseAddress + XEL_BUFFER_OFFSET) &
				XEL_TSR_XMIT_ACTIVE_MASK) != 0)) {
		...
		Register = XEmacLite_mGetTxActive(BaseAddress +
							XEL_BUFFER_OFFSET);
		Register &= ~XEL_TSR_XMIT_ACTIVE_MASK;
		XEmacLite_mSetTxActive(BaseAddress + XEL_BUFFER_OFFSET,
					Register);
	}
