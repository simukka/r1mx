...
## 6c. ISE 10.x EDK Source Library (r1mx_mmio) — Decompilation Reference

Path: `/home/simukka/src/RED/r1mx_mmio/` (Xilinx EDK 10.1 Platform Studio project)

This project was generated by Xilinx libgen for the RED ONE MX FPGA design. It contains
the full BSP driver source that was compiled into the VxWorks firmware binary. Comparing
driver source patterns against disassembly directly identifies functions.

### FPGA Chip Identification (from system.xmp)

```
Device:    Xilinx XC4VFX100
Package:   FF1517
Speed:     -11
Board:     r1mx-virtex4 (Virtex-4 FX series — contains PPC405 hard core)
EDK ver:   Xilinx EDK 10.1 EDK_K.15
```

The PPC405 is an **embedded hard macro** inside the XC4VFX100 FPGA. The entire camera
compute stack (CPU + all peripherals) lives in one FPGA chip.

### Driver Source Library (libsrc/)

All of these C sources were compiled into the VxWorks BSP and appear in the firmware binary.
Use them to identify functions by comparing instruction patterns:

| Driver | Path | Key functions |
|--------|------|---------------|
| common | `common_v1_00_a/src/` | `XBasicTypes`, memory test |
| cpu_ppc405 | `cpu_ppc405_v1_10_a/src/xio.c` | `XIo_In8/16/32`, `XIo_Out8/16/32` (MMIO wrappers) |
| cpu_ppc405 | `cpu_ppc405_v1_10_a/src/xio_dcr.c` | `XIo_DcrIn`, `XIo_DcrOut` (DCR bus wrappers) |
| dmacentral | `dmacentral_v1_10_b/src/` | `XDmaCentral_Transfer`, `_Reset`, `_GetStatus` |
| emaclite | `emaclite_v1_12_a/src/` | `XEmacLite_Send`, `_Recv`, `_CfgInitialize` |
| intc | `intc_v1_10_c/src/` | `XIntc_Connect`, `_Enable`, `_Start`, `_AckIntr` |
| iic | `iic_v1_13_b/src/` | `XIic_MasterSend`, `_MasterRecv`, `_DynMasterSend` |
| standalone | `standalone_v2_00_a/src/boot.S` | Boot vectors; `_boot0` → `_start` |
| standalone | `standalone_v2_00_a/src/cpu_init.S` | `__cpu_init`: PPC405 errata CR 203746 CCR0 patch |
| standalone | `standalone_v2_00_a/src/xvectors.S` | Exception vector table (160-byte EABI frame) |
| uartlite | `uartlite_v1_12_a/src/` | `XUartLite_Send`, `_Recv`, `_CfgInitialize` |
| uartns550 | `uartns550_v1_11_a/src/` | `XUartNs550_Send`, `_Recv`, `_SetBaudRate` |

### Confirmed Function Identifications (ISE source match)

These firmware functions were identified by matching their instruction sequence against
the corresponding Xilinx driver source in libsrc/:

| Firmware Address | Xilinx Function | Source File | Match Evidence |
|-----------------|-----------------|-------------|----------------|
| `0x0000E898` | `XIo_In32(addr)` | `xio.c` | `eieio/sync + lwz r3,0(r3) + blr` = `__asm__ volatile ("eieio; lwz %0,0(%1)")` |
| `0x0000E904` | `XIo_Out32(addr, val)` | `xio.c` | `stw r4,0(r3) + eieio/sync + blr` = `__asm__ volatile ("stw %0,0(%1); eieio")` |

**Decompilation rule:** Any `bl 0xe898` call with `r3` = MMIO address reads a 32-bit
peripheral register. Any `bl 0xe904` call writes `r4` to the MMIO register at `r3`.
Cross-reference `r3` against xparameters.h peripheral base addresses + register offsets
from the corresponding `_l.h` header to name the register.

**Example annotation:**
```asm
; XIo_Out32(XPAR_UARTLITE_0_BASEADDR + XUL_TX_FIFO_OFFSET, char)
lis  r3, 0xe060         ; r3 = 0xe0600000 (UartLite base)
addi r4, r5, 0          ; r4 = char to send
bl   0xe904             ; XIo_Out32(0xe0600004, char) → write TX FIFO
```

### Register Maps (from libsrc headers)

**UartLite at 0xE0600000** (`xuartlite_l.h`):

| Offset | Register | RW | Notes |
|--------|----------|----|-------|
| +0 | RX FIFO | R | Receive data |
| +4 | TX FIFO | W | Transmit character |
| +8 | Status | R | Bit3=TX full, Bit2=TX empty, Bit1=RX full, Bit0=RX valid |
| +12 | Control | W | Bit1=RX FIFO reset, Bit0=TX FIFO reset |

**UartNs550 at 0xE0640000 / 0xE0650000** (`xuartns550_l.h`):
Registers at `base + 0x1003` for THR, `base + 0x1007` for IER, etc.
(`XUN_REG_OFFSET = 0x1000`; each 8-bit register is at a 4-byte boundary +3 in big-endian PLB)

**INTC at 0xE0800000** (`xintc_l.h`):

| Offset | Register | Notes |
|--------|----------|-------|
| +0 | ISR | Interrupt Status |
| +4 | IPR | Interrupt Pending |
| +8 | IER | Interrupt Enable |
| +12 | IAR | Interrupt Acknowledge |
| +16 | SIE | Set Interrupt Enable |
| +20 | CIE | Clear Interrupt Enable |
| +24 | IVR | Interrupt Vector |
| +28 | MER | Master Enable (bit0=hardware enable, bit1=master enable) |

**I2C at 0xB2600000** (`xiic_l.h`):
Registers at `base + 0x103` for CR, `base + 0x107` for SR, `base + 0x10B` for TX data, etc.
(`XIIC_REG_OFFSET = 0x100 + 3`; big-endian 4-byte aligned byte registers)

**EthernetLite at 0xE1020000** (`xemaclite_l.h`):
TX buffer at base, status at `base+0x7FC`, RX buffer at `base+0x1000`, RX status at `base+0x17FC`

### PPC405 Errata Patch (from cpu_init.S, CR 203746)

The Xilinx BSP applies this patch in `__cpu_init()`:
```asm
mfccr0  r3
oris    r3, r3, 0x5000    ; set CCR0[1] and CCR0[3] (bits 0x40000000 and 0x10000000)
mtccr0  r3
```
This addresses PPC405 errata where certain cache operations mis-behave without these bits.
Search for this pattern in firmware boot code (0x0000-0x00A4) to confirm errata is applied.

### SPR Register Names (from xreg405.h) — for annotating mtspr/mfspr

| SPR# (hex) | Name | Notes |
|------------|------|-------|
| 0x10c | TBL_READ | Time Base Lower (read) |
| 0x10d | TBU_READ | Time Base Upper (read) |
| 0x11c | TBL_WRITE | Time Base Lower (write) |
| 0x11d | TBU_WRITE | Time Base Upper (write) |
| 0x3b3 | CCR0 | Core config: cache/prefetch/PLB priority |
| 0x3b9 | SGR | Storage Guarded Register |
| 0x3ba | DCWR | Data Cache Write-Through Register |
| 0x3bb | SLER | Storage Little-Endian Register |
| 0x3bc | SU0R | Storage User 0 Register |
| 0x3d6 | EVPR | Exception Vector Prefix (upper 16 bits of vector base) |
| 0x3d8 | TSR | Timer Status Register (WDT/PIT/FIT status bits) |
| 0x3da | TCR | Timer Control Register (WDT/PIT/FIT config) |
| 0x3db | PIT | Programmable Interval Timer |
| 0x3fa | DCCR | Data Cache Cacheable Region Register |
| 0x3fb | ICCR | Instruction Cache Cacheable Region Register |

**Key TCR bits (at 0x3da):**
- Bits[0:1] = WDT period (11 = longest)
- Bits[2:3] = WDT reset control (11 = system reset on overflow)
- Bit[4] = WDT interrupt enable
- Bit[5] = PIT interrupt enable
- Bit[8:9] = FIT period
- Bit[10] = FIT interrupt enable
- Bit[11] = PIT auto-reload

**Key TSR bits (at 0x3d8):**
- Bit[0] = WDT enable-next-watchdog
- Bit[1] = WDT interrupt status
- Bits[2:3] = WDT reset status
- Bit[4] = PIT interrupt status
- Bit[5] = FIT interrupt status

When you see `TSR = 0xC0000000` (bits 0:1) = WDT fired + WDT enable; this is the boot
loop watchdog trigger we observed (18701 resets per session).
...
### Xilinx BSP MMIO / DCR I/O Wrappers (confirmed from ISE libsrc)

These small leaf functions were compiled from `cpu_ppc405_v1_10_a/src/xio.c` and appear
literally throughout the firmware wherever driver code accesses peripheral registers.

| Address | Xilinx Function | Signature | Instruction pattern |
|---------|-----------------|-----------|---------------------|
| `0x0000E898` | `XIo_In32` | `u32 XIo_In32(u32 addr)` | `sync; lwz r3,0(r3); blr` |
| `0x0000E904` | `XIo_Out32` | `void XIo_Out32(u32 addr, u32 val)` | `stw r4,0(r3); sync; blr` |

**Usage:** Whenever you see `bl 0xe898` with `r3=MMIO_addr`, it is a 32-bit peripheral
register read. `bl 0xe904` with `r3=MMIO_addr, r4=value` is a 32-bit peripheral write.
Look up `r3` against Section 6 MMIO map and the `_l.h` register offset tables (Section 6c).

There are likely also `XIo_In8/16` and `XIo_Out8/16` variants at nearby addresses
(pattern: `eieio/sync + lbz/lhz` for read, `stb/sth + eieio/sync` for write).
